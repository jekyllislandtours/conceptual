...
public class IntArrayList implements Counted, Seqable, Indexed {
    ...
    static int sumSizes(final int[] a, final int[] b, final int[] c, final int[] d,
                               final int[] e, final int[] f, final int[] g, final int[] h) {
        int n = null==a ? 0 : a.length;
        n += null==b ? 0 : b.length;
        n += null==c ? 0 : c.length;
        n += null==d ? 0 : d.length;
        n += null==e ? 0 : e.length;
    ...
    static void addArrays(IntArrayList ial, final int[] a, final int[] b, final int[] c, final int[] d,
                                 final int[] e, final int[] f, final int[] g, final int[] h) {
        final int numToAdd = sumSizes(a, b, c, d, e, f, g, h);
        ial.ensureCapacity(ial.size + numToAdd);
        if (null != a) {
            System.arraycopy(a, 0, ial.data, ial.size, a.length);
            ial.size += a.length;
        }
        if (null != b) {
            System.arraycopy(b, 0, ial.data, ial.size, b.length);
            ial.size += b.length;
        }
        if (null != c) {
            System.arraycopy(c, 0, ial.data, ial.size, c.length);
            ial.size += c.length;
        }

        if (null != d) {
            System.arraycopy(d, 0, ial.data, ial.size, d.length);
            ial.size += d.length;
        }

        if (null != e) {
            System.arraycopy(e, 0, ial.data, ial.size, e.length);
            ial.size += e.length;
        }

        if (null != f) {
            System.arraycopy(f, 0, ial.data, ial.size, f.length);
            ial.size += f.length;
        }

        if (null != g) {
            System.arraycopy(g, 0, ial.data, ial.size, g.length);
            ial.size += g.length;
        }

        if (null != h) {
            System.arraycopy(h, 0, ial.data, ial.size, h.length);
            ial.size += h.length;
        }
    }

    static void addAllArrays(IntArrayList ial, final int[]... arrays) {
        final int partitionSize = 8;
        if (arrays == null || arrays.length == 0) { return; }
        List<int[]> l = new ArrayList<>(partitionSize);
        for (int i=0; i < arrays.length; i++) {
            if (i % partitionSize == 0) {
                if (!l.isEmpty()) {
                    addArrays(ial, l.get(0), l.get(1), l.get(2), l.get(3),
        ...
        // add elements so that the gets don't blow up
        for (int i = l.size(); i < partitionSize; i++) {
            l.add(null);
        }
    ...
    public void addArrays(final int[] a, final int[] b) {
        addArrays(a, b, null, null, null, null, null, null);
    }

    public void addArrays(final int[] a, final int[] b, final int[] c) {
        addArrays(a, b, c, null, null, null, null, null);
    }

    public void addArrays(final int[] a, final int[] b, final int[] c, final int[] d) {
        addArrays(a, b, c, d, null, null, null, null);
    }

    public void addArrays(final int[] a, final int[] b, final int[] c, final int[] d,
                          final int[] e) {
        addArrays(a, b, c, d, e, null, null, null);
    }

    public void addArrays(final int[] a, final int[] b, final int[] c, final int[] d,
                          final int[] e, final int[] f) {
        addArrays(a, b, c, d, e, f, null, null);
    }

    public void addArrays(final int[] a, final int[] b, final int[] c, final int[] d,
                          final int[] e, final int[] f, final int[] g) {
        addArrays(a, b, c, d, e, f, g, null);
    }


    public void addArrays(final int[] a, final int[] b, final int[] c, final int[] d,
                          final int[] e, final int[] f, final int[] g, final int[] h, final int[]... more) {
        addArrays(this, a, b, c, d, e, f, g, h);
        addAllArrays(this, more);
    }
